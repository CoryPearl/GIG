...
app.get('/login', (req, res) => {
  const redirect_uri = `http://${ip}:${PORT}/callback`;
  const scope = 'repo';
...
app.post('/sendRepoData', async (req, res) => {
  ...
  for (const e of repos) {
    const repoInfo =
      ...
      ' | README: ' +
      (e.readme ? e.readme.slice(0, 2000) : 'No README found') +
      '\n\n';
    fullInfo += repoInfo;
  }
  ...
  if (option === '1') {
    response = await gemini.askGemini(
      `I’m going to give you a list of programming and computer science projects I’ve already completed.
      Based on that list, generate a bulleted list of completely new and original project ideas that build on my past experience 
      but explore new directions. Each item should just be the project title or concept — no descriptions or extra info. Only about 10 or so.
      Here’s my list of past projects:\n${fullInfo}`
    );
  } else {
    response = await gemini.askGemini(
